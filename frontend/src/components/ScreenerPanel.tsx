    ...
    <Panel title="智慧選股" icon="🔍"
      right={
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button className="btn primary" onClick={run} disabled={loading}>執行</button>
        </div>
      }>
      {loading ? <div className="spinner">量化初篩中…</div> : (
        <table className="grid">
          <thead>
            <tr><th>#</th><th>代碼</th><th>綜合分</th><th>動能20</th><th>法人淨買</th><th>營收YoY</th></tr>
          </thead>
          <tbody>
            {rows.map((r) => (
              <tr key={r.stock_id} onClick={() => onSelect(r.stock_id)} style={{ cursor: "pointer" }}>
                <td>{r.rank}</td>
                <td><b>{r.stock_id}</b> <span style={{ color: "var(--text-dim)" }}>{r.stock_name}</span></td>
            ...
            {rows.length === 0 && <tr><td colSpan={6} className="empty-hint">選日期後按「執行」</td></tr>}
          </tbody>
        </table>
      )}
    </Panel>
