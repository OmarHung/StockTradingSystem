...
```
資料層 ── shioaji股價(主) + TWSE/TPEx官方籌碼除權息(主) + FinMind(月營收主源/全面備援+402自動等待)
...
1. **⚙️ 設定 → 🔑 API 金鑰**：填 Anthropic（LLM 必需）、永豐 shioaji（股價主源+排行）、FinMind（月營收）
...
| 📈 K 線圖 | 還原價、日/週/月切換（lightweight-charts） |
...
| 🗂 股票（TopBar） | 全市場 3115 檔瀏覽器：產業/市場/狀態篩選、搜尋、單股數據詳情 |
| 📦 資料（TopBar） | 健康報告(覆蓋率/新鮮度/建議)、分類型回補+逐檔進度、品質檢查 |
...
| `FINMIND_TOKEN` | 月營收 + 備援 | finmindtrade.com 免費註冊 |
